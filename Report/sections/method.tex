...
\Function{observations}{$h$, downbeat, nextDownbeat}
	...
	\For {$i \leftarrow 0,d$}
		\If {$O_i \neq *$ \textbf{and} $i \neq 0$}
			\State \textbf{append} \Call{log\textunderscore beat\textunderscore ratio}{downbeat, nextDownbeat, $B_i$, $i$, $d$} \textbf{to} $\mathcal{P}$
		\EndIf	
		\State $H', B', O' \leftarrow H_i$
		\If {$H' \neq \emptyset$}
			...
			\EndIf
			\State \textbf{append} \Call{observations}{$(H', B', O'), b_{\mathrm{down}}, b_{\mathrm{up}}$} \textbf{to} $\mathcal{P}$
		\EndIf
	\EndFor
